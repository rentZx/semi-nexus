    ...
    <div>
      ...
      <GlassCard className="mb-6">
        <div className="grid gap-4 xl:grid-cols-[1fr_0.95fr]">
          ...
          <div className="rounded-2xl border border-accent/20 bg-card-hover px-4 py-3 text-sm text-body">
            ...
            <GlassCard key={company.code} className="transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg">
              <div className="flex items-start justify-between gap-3">
                <Link to={`/companies/${company.code}`}>
                  <p className="text-sm font-semibold text-accent">{company.code} · {company.market}</p>
                  <h2 className="mt-1 text-xl font-semibold text-heading">{company.name}</h2>
                </Link>
                <button type="button" onClick={() => toggleFavorite(company.code)} className="rounded-full bg-card-hover p-2 text-amber ring-1 ring-amber/20 transition hover:ring-amber/40" aria-label="收藏公司">
                  <Star className="h-4 w-4" fill={favorites.includes(company.code) ? "currentColor" : "none"} />
                </button>
              </div>
              <p className="mt-3 text-sm leading-6 text-body">{company.mainBusiness}</p>
              <p className="mt-3 text-xs text-heading0">所属环节：{segmentNames}</p>
              <p className="mt-3 text-xs text-amber">它不是：{company.notThis[0]}</p>
